...
//    }
//
//    public String uploadFileToFirebase(File file, String fileName) throws IOException {
//
...
//
//            return this.uploadFileToFirebase(this.convertToFile(file, newFileName), newFileName);
//        } catch (Exception e) {
//            e.printStackTrace();
...
public class FirebaseService {
    ...
    public FirebaseService() {
        this.storage = initializeFirebaseStorage();
    }

    // Initialize Firebase Storage only once
    ...
    public String upload(String fileName, MultipartFile file) {
        try {
            ...
            if (file.getOriginalFilename() == null)
                throw new Exception("Original file name not found. Cannot retrieve extension.");
    ...
    public void addFilePathToUser(String email, String subject, String topicName, String fileName, String filePath, boolean isImage) {
        ...
        if (userOptional.isPresent()) {
            ...
            List<topicEntity> topicList = userTopicMap.getSubToTopicsMap().get(subject);
            if (topicList == null)
                throw new RuntimeException("No Subject registered with name: " + subject);

            for (topicEntity topic : topicList) {
                if (topic.getTopicName().equals(topicName)) {
                    if (isImage) {
                        topic.getImagePath().add(filePath);
                    } else {
                        topic.getFilePath().add(new FileEntity(fileName, filePath));
                    }
                    break;
                }
            }
